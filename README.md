...
- 累计盈亏曲线、每日盈亏柱状图
- K 线图：导入 ATAS 当日 K 线 CSV，支持 1m / 5m 切换和鼠标滚轮缩放；K 线和交易记录按 CME/Chicago 日期匹配，图表时间仍按北京时间显示，按进出场时间和点位自动标注交易，并在图表上方显示该日期的手写每日总结
...
注意：交易记录和 ATAS K 线可以继续使用北京时间。网页内部会把北京时间换算到 CME/Chicago 日期做交易日归属，例如北京时间周六 00:30 会归到 CME 周五/交易所当天；图表横轴、进出场时间和提示框仍按北京时间显示。
