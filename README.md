...
- 每日总记录、周统计、月历统计
- 总览月历联动每日总结：点击日历日期后，只切换下方空白总结输入框，不跳转交易日志；总结由用户手写，不自动生成
- 累计盈亏曲线、每日盈亏柱状图
- K 线图：导入 ATAS 当日 K 线 CSV，支持 1m / 5m 切换和鼠标滚轮缩放；只显示同时存在交易记录和 K 线数据的交易日，按进出场时间和点位自动标注交易，并在图表上方显示该日期的手写每日总结
...
1. 点击月历中的任意日期。
2. 下方“每日总结”只切换到该日期的空白输入框，不跳转到交易日志。
