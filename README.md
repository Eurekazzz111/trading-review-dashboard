...
- 累计盈亏曲线、每日盈亏柱状图
- K 线图：导入 ATAS 当日 K 线 CSV，支持 1m / 5m 切换和鼠标滚轮缩放；ATAS 时间按 CME/Chicago 解析并按 CME 交易日匹配，图表、交易日期和进出场时间均按北京时间显示
...
ATAS 导出格式为 `年-日-月 时间;开;高;低;收`（分号分隔，可含多天，无表头），其中日期时间必须是 CME/Chicago 交易所时间。网页会在导入时自动换算为北京时间显示，并按 CME 交易日保存和匹配。
...
注意：交易 CSV 的 `date`、`entryTime` 和 `exitTime` 一律按北京时间填写和显示。导入时网页会将入场时间换算为 CME/Chicago 交易日来匹配 K 线：例如北京时间周六 00:30 对应 Chicago 周五 11:30，会匹配 CME 周五交易日的 K 线；图表横轴、进出场时间、交易日期和提示框仍保持北京时间。
